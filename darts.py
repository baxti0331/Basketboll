...
def init_db():
    with sqlite3.connect("darts_bot.db") as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def darts_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text="🎲 Кости ", url="https://t.me/kostigivsbot")
...
async def process_referral(callback_query: CallbackQuery):
    ...
    text = (
        "🎯 Возможность попасть в цель!\n"
        "🎁 Подарки за каждый бросок!\n\n"
        "👫 Пригласите друзей:\n"
...
async def back_to_menu(callback_query: CallbackQuery):
    await callback_query.message.edit_text(
        "🎯 Подарки для дартса\n\n"
        "Побеждайте с каждым броском\n"
        "и получайте отличные подарки 🧸💝🎁🌹\n\n"
        f"💰 Баланс: {get_user_stars(callback_query.from_user.id)} ⭐️",
        reply_markup=darts_keyboard(callback_query.from_user.id)
    )
    await callback_query.answer()
...
async def process_throw(callback_query: CallbackQuery):
    ...
    if price_stars is None:
        await callback_query.answer("⛔ Некорректное количество бросков")
        return

    await bot.send_invoice(
        chat_id=user_id,
        title=f"Оплата за {count} бросков",
        description=f"Дартс: {count} бросков",
        payload=f"darts_{count}",
        provider_token=PROVIDER_TOKEN,
        currency="XTR",
        prices=[LabeledPrice(label=f"{count} бросков", amount=price_stars)],
        start_parameter=f"darts_{count}"
    )
    await callback_query.answer()
...
async def successful_payment(msg: types.Message):
    ...
    if payload.startswith("darts_"):
        ...
        if hits == count:
            ...
            await bot.send_message(user_id, "🎉 Все броски попали в цель!")
            await asyncio.sleep(0.5)
        ...
        await bot.send_message(user_id, f"💰 Баланс: {stars} ⭐️", reply_markup=darts_keyboard(user_id))
...
async def send_menu_with_admin(user_id: int, chat_id: int):
    ...
    await bot.send_message(chat_id=chat_id, text=text, reply_markup=darts_keyboard(user_id))
