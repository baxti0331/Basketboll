...
def football_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    for count in sorted(COSTS.keys(), reverse=True):
        builder.button(
            text=f"⚽ {count} зарба • {COSTS[count]}⭐️",
            callback_data=f"kick_{count}"
        )
    builder.button(text="+ 3 ⭐️ дўст учун", callback_data=f"referral_{user_id}")
    builder.button(text="🎁 Совғалар 🏆", url="https://t.me/bankstars_support_bot")
    if user_id == ADMIN_ID and ADMIN_ID != 0:
        builder.button(text="⚙️ Админ панели", callback_data="admin_menu")
    builder.adjust(2)
...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="📢 Хабарнома", callback_data="admin_broadcast"),
        ],
        [
            InlineKeyboardButton(text="⬅️ Орқага", callback_data="back_to_menu"),
        ]
    ])
...
def admin_stats_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="⬅️ Орқага", callback_data="admin_menu"),
        ]
    ])
...
async def admin_menu_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "admin_menu":
        ...
        text = (
            ...
            f"Даромад: {income} XTR\n"
            f"Харажат: {expense} XTR"
        )
        await callback.message.edit_text(text, reply_markup=admin_stats_keyboard())
        await callback.answer()

    elif data == "admin_broadcast":
        await callback.message.edit_text("📝 Хабарнома учун матн ёки медиа юборинг ёки 'йўқ' деб жавоб беринг.")
...
async def process_referral(callback_query: CallbackQuery):
    ...
    text = (
        "⚽ Тўпни мақсадга йўллаш учун имконият!\n"
        "🎁 Ҳар бир зарба учун совғалар!\n\n"
        "👫 Дўстларни таклиф этинг:\n"
        "— Ҳар бир дўст учун +3 ⭐️\n\n"
        "🔥 Сиз учун имконият бу ерда!\n"
        f"{url}"
    )

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📤 Улашиш", switch_inline_query=text)],
        [InlineKeyboardButton(text="⬅️ Орқага", callback_data="back_to_menu")]
    ])
...
async def back_to_menu(callback_query: CallbackQuery):
    await callback_query.message.edit_text(
        "⚽ Футбол учун совғалар\n\n"
        "Ҳар бир зарбада гол уринг \n"
        "ва ажойиб совғаларга эга болинг  🧸💝🎁🌹\n\n"
...
async def process_kick(callback_query: CallbackQuery):
    ...
    if price_stars is None:
        await callback_query.answer("⛔ Нотўғри зарба сони")
        return

    await bot.send_invoice(
        chat_id=user_id,
        title=f"{count} зарба учун тўлов",
        description=f"Футбол ўйини учун {count} зарба",
        payload=f"football_{count}",
        provider_token=PROVIDER_TOKEN,
        currency="XTR",
        prices=[LabeledPrice(label=f"{count} зарба", amount=price_stars)],
    ...
    if payload.startswith("football_"):
        ...
        for i in range(count):
            ...
            if dice.dice.value >= 3:
                ...
                results.append((i + 1, "гол эмас ❌"))

        throws_text = "\n".join([f"Зарба #{i} – {res}" for i, res in results])
        quote_msg = f"Ўйин натижалари ⚽ {count} зарба\n\n{throws_text}"
        await bot.send_message(user_id, f"```{quote_msg}```", parse_mode="Markdown")

        if goals == count:
            await asyncio.sleep(0.5)
            await bot.send_message(user_id, "🎉 Барча зарбалар голга айланди!")
            await asyncio.sleep(0.5)
            await bot.send_message(user_id, "🎁 Совғангиз йўлланмоқда...")
            ...
            if GIFT_IDS:
                ...
                try:
                    ...
                    logging.error(f"Совға юборишда хатолик {gift_to_send} учун {user_id}: {e}")

            await asyncio.sleep(0.5)
            await bot.send_message(user_id, "")
        else:
            await bot.send_message(user_id, "Қайта уриниб кўринг!")
...
async def process_broadcast_media(message: types.Message, state: FSMContext):
    if message.text and message.text.lower() == "йўқ":
        await message.answer("Хабар матнини юборинг.")
...
async def process_broadcast_content(message: types.Message, state: FSMContext):
    ...
    await message.answer("Тугма матнини юборинг ёки 'йўқ' деб жавоб беринг.")
...
async def process_broadcast_button_text(message: types.Message, state: FSMContext):
    ...
    if text.lower() == "йўқ":
        ...
        await message.answer("Тугма учун ҳаволани юборинг.")
...
async def send_broadcast(state: FSMContext, message: types.Message):
    ...
    for user_id in users:
        try:
            ...
            logging.error(f"Хатолик юборишда {user_id}га: {e}")

    await message.answer("✅ Хабарнома юборилди.")
    await state.clear()

async def send_menu_with_admin(user_id: int, chat_id: int):
    text = (
        "⚽ Футбол учун совғалар\n\n"
        "Ҳар бир зарбада ғалаба қозонинг\n"
        "ва ажойиб совғаларни олинг 🧸💝🎁🌹\n\n"
