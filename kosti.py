...
def init_db():
    with sqlite3.connect("dice_bot.db") as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def dice_keyboard(user_id: int) -> InlineKeyboardMarkup:
...
async def back_to_menu(callback_query: CallbackQuery):
    await callback_query.message.edit_text(
        "🎲 Подарки для кубиков\n\n"
        "Испытай удачу с каждым броском\n"
        "и получайте отличные подарки 🧸💝🎁🌹\n\n"
        f"💰 Баланс: {get_user_stars(callback_query.from_user.id)} ⭐️",
        reply_markup=dice_keyboard(callback_query.from_user.id)
    )
    await callback_query.answer()

@dp.callback_query(F.data.startswith("roll_"))
async def process_roll(callback_query: CallbackQuery):
    ...
    await bot.send_invoice(
        ...
        description=f"Кубики: {count} бросков",
        payload=f"dice_{count}",
        provider_token=PROVIDER_TOKEN,
        currency="XTR",
        prices=[LabeledPrice(label=f"{count} бросков", amount=price_stars)],
        start_parameter=f"dice_{count}"
    )
    await callback_query.answer()
...
async def successful_payment(msg: types.Message):
    ...
    if payload.startswith("dice_"):
        ...
        if wins == count:
            ...
            await bot.send_message(user_id, "🎉 Все броски выигрышные!")
            await asyncio.sleep(0.5)
        ...
        await bot.send_message(user_id, f"💰 Баланс: {stars} ⭐️", reply_markup=dice_keyboard(user_id))
...
async def send_menu_with_admin(user_id: int, chat_id: int):
    ...
    await bot.send_message(chat_id=chat_id, text=text, reply_markup=dice_keyboard(user_id))
